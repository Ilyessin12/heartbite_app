...
class _TestSupabaseScreenState extends State<TestSupabaseScreen> {
  ...
  List<Map<String, dynamic>> _recipes = []; // State for recipes
  ...
  Future<void> fetchUsers() async {
    ...
    try {
      ...
      setState(() { _statusMessage = 'Error loading users: $e'; });
      print('Error fetching users: $e');
    } finally {
      // Keep _isLoading true if we are about to fetch recipes, otherwise set to false
      if (mounted && _recipes.isEmpty) {
         //  setState(() { _isLoading = false; }); // Will be set by _fetchRecipes
      } else if (mounted) {
  ...
  Future<void> createUser() async {
    if (_fullNameController.text.isEmpty || _usernameController.text.isEmpty || _emailController.text.isEmpty || _passwordController.text.isEmpty) {
      setState(() { _statusMessage = 'Please fill all required user fields'; });
      return;
    }
    setState(() { _isLoading = true; _statusMessage = 'Creating user...'; });
    try {
      ...
      if (authResponse.user != null) {
        ...
        setState(() { _statusMessage = 'User created successfully'; });
  ...
  Future<void> updateUser(String userId) async {
    if (_updateFullNameController.text.isEmpty) {
      setState(() { _statusMessage = 'Full name is required for user update'; });
      return;
    }
    setState(() { _isLoading = true; _statusMessage = 'Updating user...'; });
    try {
      ...
      setState(() { _statusMessage = 'User updated successfully'; });
      ...
      setState(() { _statusMessage = 'Error updating user: $e'; });
  ...
  Future<void> deleteUser(String userId) async {
    setState(() { _isLoading = true; _statusMessage = 'Deleting user...'; });
    try {
      await _supabase.from('users').delete().eq('id', userId);
      setState(() { _statusMessage = 'User deleted successfully'; });
      await fetchUsers();
    } catch (e) {
      setState(() { _statusMessage = 'Error deleting user: $e'; });
  ...
  Future<void> _fetchRecipes() async {
    ...
    try {
      final response = await _supabase.from('recipes').select('*, users(username)').order('created_at', ascending: false);
      setState(() {
  ...
  Future<void> _createRecipe() async {
    if (_recipeTitleController.text.isEmpty || _recipeCookingTimeController.text.isEmpty || _recipeServingsController.text.isEmpty) {
      setState(() { _statusMessage = 'Recipe title, cooking time, and servings are required.'; });
      return;
    }
    setState(() { _isLoading = true; _statusMessage = 'Creating recipe...'; });
    try {
      ...
      setState(() { _statusMessage = 'Recipe created successfully'; });
      await _fetchRecipes();
    } catch (e) {
      setState(() { _statusMessage = 'Error creating recipe: $e'; });
  ...
  Future<void> _updateRecipe(int recipeId) async {
     if (_updateRecipeTitleController.text.isEmpty || _updateRecipeCookingTimeController.text.isEmpty || _updateRecipeServingsController.text.isEmpty) {
      setState(() { _statusMessage = 'Recipe title, cooking time, and servings are required for update.'; });
      return;
    }
    setState(() { _isLoading = true; _statusMessage = 'Updating recipe...'; });
    try {
      ...
      setState(() { _statusMessage = 'Recipe updated successfully'; });
      ...
      setState(() { _statusMessage = 'Error updating recipe: $e'; });
  ...
  Future<void> _deleteRecipe(int recipeId) async {
    setState(() { _isLoading = true; _statusMessage = 'Deleting recipe...'; });
    try {
      await _supabase.from('recipes').delete().eq('id', recipeId);
      setState(() { _statusMessage = 'Recipe deleted successfully'; });
      await _fetchRecipes();
    } catch (e) {
      setState(() { _statusMessage = 'Error deleting recipe: $e'; });
                  ...
                  ExpansionTile(
                    ...
                    children: [
                      ...
                      ExpansionTile(
                        ...
                        initiallyExpanded: false,
                        children: [
                          ListView.builder( // This ListView should be constrained or use shrinkWrap+physics
                            shrinkWrap: true,
